...
func TestProjector(t *testing.T) {
	...
	t.Run("subtracts_the_scaled_mean_of_a_unit_input", func(t *testing.T) {
		...
		// {0, 1} - 0.5 * {1, 0} = {-0.5, 1}, left unnormalized: the residual's
		// length is what weighs a member in its story's centroid.
		assert.InDelta(t, -0.5, got[0], 1e-6)
		assert.InDelta(t, 1.0, got[1], 1e-6)
		assert.InDelta(t, math.Sqrt(1.25), dist.Norm(got), 1e-6)
	})
...
func TestProjectInPlace(t *testing.T) {
	...
	// The residual is not renormalized. Renormalizing would make every member
	// weigh the same in Centroid and move every cluster boundary in the corpus.
	assert.InDelta(t, -0.5, v[0], 1e-6)
	assert.InDelta(t, 1.0, v[1], 1e-6)
	assert.InDelta(t, math.Sqrt(1.25), dist.Norm(v), 1e-6)
}
